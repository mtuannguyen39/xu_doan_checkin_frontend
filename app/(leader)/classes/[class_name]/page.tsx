...
function AttendanceDots({ rate }: { rate: number }) {
  const color =
    rate >= 80 ? "#10b981" : rate >= 60 ? "#f59e0b" : rate >= 40 ? "#f97316" : "#ef4444";
  return (
      ...
      <div className="relative w-8 h-8">
        <svg viewBox="0 0 36 36" className="rotate-90 w-8 h-8">
          ...
          <circle cx="18" cy="18" r="15" fill="none" stroke={color} strokeWidth="3"
            strokeDasharray={`${(rate / 100) * 94.25} 94.25`} strokeLinecap="round" />
        </svg>
...
function EditModal({ student, onSave, onCancel, loading }: {
  student: Student;
  onSave: (data: { full_name: string; saint_name: string; phone: string; is_active: boolean }) => void;
  onCancel: () => void;
  loading: boolean;
}) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4"
      ...
      <div className="w-full max-w-sm rounded-2xl border border-white/15 p-6"
        ...
        <p className="text-white/40 text-xs mb-5">ID: {student.id} · Lớp {student.class_name}</p>
        <div className="space-y-4">
          <div>
            <label className="text-white/50 text-xs font-semibold mb-1.5 block">Họ và tên</label>
          ...
          <div>
            <label className="text-white/50 text-xs font-semibold mb-1.5 block">Tên Thánh</label>
            <input value={form.saint_name} onChange={(e) => setForm({ ...form, saint_name: e.target.value })}
              className="w-full rounded-xl bg-white/5 border border-white/10 px-4 py-2.5 text-white text-sm outline-none focus:border-blue-500/50" />
          </div>
          <div>
            <label className="text-white/50 text-xs font-semibold mb-1.5 block">Số điện thoại</label>
            <input value={form.phone} placeholder="Không bắt buộc"
          ...
          <div className="flex items-center justify-between px-4 py-3 rounded-xl bg-white/5 border border-white/10">
            <div>
              ...
              <p className="text-white/40 text-xs">Thiếu nhi có đang hoạt động?</p>
            </div>
        ...
        <p className="text-white/20 text-xs mt-4 mb-5">* Không thể đổi lớp, ngành hoặc ID qua form này</p>
        <div className="flex gap-3">
          ...
          <button onClick={() => onSave(form)} disabled={loading || !form.full_name.trim() || !form.saint_name.trim()}
...
function DeleteModal({ name, onConfirm, onCancel, loading }: {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4"
      style={{ background: "rgba(0,0,0,0.75)", backdropFilter: "blur(8px)" }}>
      <div className="w-full max-w-sm rounded-2xl border border-red-500/20 p-6"
        style={{ background: "rgba(15,3,3,0.97)" }}>
        <p className="text-3xl text-center mb-3">⚠️</p>
        <h3 className="text-white font-black text-center text-lg mb-2">Xác nhận xóa</h3>
        <p className="text-white/50 text-sm text-center mb-6">
          Bạn có chắc muốn xóa <span className="text-white font-semibold">{name}</span>?<br />
          <span className="text-red-400 text-xs">Toàn bộ lịch sử điểm danh sẽ bị xóa vĩnh viễn!</span>
        </p>
        <div className="flex gap-3">
          <button onClick={onCancel} disabled={loading}
            className="flex-1 py-2.5 rounded-xl bg-white/5 border border-white/10 text-white/60 font-semibold text-sm hover:bg-white/10 transition-all disabled:opacity-50">
...
// ── Student Row ──────────────────────────────────────────────
function StudentRow({ student, canEdit, onEdit, onDelete, onShowQR, onEditPoint }: {
  student: Student;
  canEdit: boolean;
  ...
  return (
    <>
      <tr className="border-b border-white/5 hover:bg-white/5 transition-colors">
        ...
        <td className="px-4 py-3 text-center cursor-pointer" onClick={() => setExpanded(!expanded)}>
        ...
        <td className="px-4 py-3 text-center cursor-pointer" onClick={() => setExpanded(!expanded)}>
          <div className="flex gap-1 justify-center">
            {student.recent_checkins.slice(0, 5).map((c, i) => (
              <div key={i} title={new Date(c.date).toLocaleDateString("vi")}
          ...
          <div className="flex items-center justify-end gap-1.5">
            {canEdit && (
              <>
                ...
                <button onClick={(e) => { e.stopPropagation(); onDelete(student.id, student.full_name); }}
            ...
            <button onClick={(e) => { e.stopPropagation(); onShowQR(student.id); }}
            ...
            <span className={`text-white/30 text-xs transition-transform inline-block cursor-pointer select-none ${expanded ? "rotate-180" : ""}`}
                    ...
                    <div>
                      ...
                      <p className={`text-sm font-black mt-0.5 ${
                        ...
                        <span className="text-white/30 font-normal text-xs ml-1.5">
                          {c.point === 5 ? "Đúng giờ" : c.point === 2 ? "Trễ nhẹ" : "Trễ"}
                        </span>
                      </p>
                    </div>
...
export default function ClassDetailPage() {
  ...
  const { exportClass, exportDay, exporting } = useExportExcel();
  ...
  const [editPointTarget, setEditPointTarget] = useState<EditPointTarget | null>(null);

  const [toast, setToast] = useState<{ msg: string; type: "success" | "error" } | null>(null);

  const canEdit = ["SUPER_ADMIN", "XU_DOAN_TRUONG", "XU_DOAN_PHO", "TRUONG_TRUC", "TRUONG_LOP"].includes(user?.role ?? "");
  ...
  useEffect(() => {
    api.get(`/leaderboard/classes/${encodeURIComponent(className)}`)
      .then((res) => { setClassData(res.data.class); setStudents(res.data.students); })
      .catch(console.error)
      .finally(() => setLoading(false));
  }, [className]);

  const handleEditSave = async (data: { full_name: string; saint_name: string; phone: string; is_active: boolean }) => {
    if (!editTarget) return;
    setEditLoading(true);
    try {
      const res = await api.patch(`/students/${editTarget.id}`, data);
      setStudents((prev) => prev.map((s) => s.id === editTarget.id ? { ...s, ...res.data.data } : s));
    ...
    <ProtectedRoute>
      <div className="min-h-screen bg-[#080c14] text-white">
        <div className="max-w-6xl mx-auto px-4 py-8">
          ...
          {/* Toast */}
          {toast && (
            <div className={`fixed top-4 right-4 z-50 px-4 py-3 rounded-xl text-sm font-semibold shadow-2xl ${
              toast.type === "success"
          ...
          {/* Modals */}
          {qrTarget && <QRModal studentId={qrTarget.id} onClose={() => setQrTarget(null)} />}
          ...
          {editPointTarget && (
            <EditPointModal
              ...
              onSaved={(checkinId, newPoint) => {
                ...
                setClassData((prev) => prev ? { ...prev, total_points: prev.total_points + pointDiff } : prev);
          ...
          {editTarget && (
            <EditModal student={editTarget} onSave={handleEditSave}
              onCancel={() => setEditTarget(null)} loading={editLoading} />
          )}

          {deleteTarget && (
            <DeleteModal name={deleteTarget.name} onConfirm={handleDeleteConfirm}
          ...
          {loading ? (
            ...
            <>
                ...
                <div className="rounded-2xl bg-linear-to-br from-blue-500/10 to-indigo-600/5 border border-blue-500/20 p-6 mb-6">
                  <div className="flex flex-wrap items-start justify-between gap-4">
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                      <StatCard label="Thiếu nhi"     value={students.length}               unit=""  />
              ...
              <div className="flex flex-wrap gap-3 mb-4">
                ...
                {/* ── Xuất Excel toàn bộ ── */}
                <button
                  onClick={async () => { try { await exportClass(className); } catch (e: any) { showToast(e.message, "error"); } }}
                  disabled={exporting}
                  ...
                  {showDatePicker ? (
                    <>
                      <input
                        ...
                        className="px-3 py-2 rounded-xl bg-white/5 border border-blue-500/30 text-white text-sm outline-none focus:border-blue-500/60 scheme:dark"
                      />
                      <button
                        onClick={async () => {
                          if (!exportDate) { showToast("Vui lòng chọn ngày!", "error"); return; }
                          try {
                            await exportDay(className, exportDate);
                            setShowDatePicker(false);
                            setExportDate("");
                          } catch (e: any) { showToast(e.message, "error"); }
                        }}
                        disabled={exporting || !exportDate}
                        className="flex items-center gap-1.5 px-4 py-2 rounded-xl bg-blue-500 text-white text-sm font-bold hover:bg-blue-600 transition-all disabled:opacity-50 whitespace-nowrap"
                      >
                        {exporting
                          ? <span className="w-3 h-3 border-2 border-white/30 border-t-white rounded-full animate-spin" />
                          : "📅"}
                        {exporting ? "Đang xuất..." : "Xuất ngày này"}
                      </button>
                      <button
                        onClick={() => { setShowDatePicker(false); setExportDate(""); }}
                        className="px-3 py-2 rounded-xl bg-white/5 border border-white/10 text-white/40 text-sm hover:bg-white/10 transition-all"
                      >
                        ✕
                      </button>
                    </>
                  ...
                  <table className="w-full">
                    <thead>
                      <tr className="border-b border-white/10">
                        <th className="px-4 py-3 text-left text-xs font-semibold text-white/30 uppercase w-12">#</th>
                        <th className="px-4 py-3 text-left text-xs font-semibold text-white/30 uppercase">Học sinh</th>
                        <th className="px-4 py-3 text-left text-xs font-semibold text-white/30 uppercase">Chuyên cần</th>
                        <th className="px-4 py-3 text-center text-xs font-semibold text-white/30 uppercase">Buổi ĐD</th>
                        <th className="px-4 py-3 text-center text-xs font-semibold text-white/30 uppercase">Điểm</th>
                        <th className="px-4 py-3 text-center text-xs font-semibold text-white/30 uppercase">5 buổi gần nhất</th>
                    ...
                    <tbody>
                      {filtered.map((student) => (
                        <StudentRow
                          ...
                          onShowQR={(id) => setQrTarget({ id, name: student.full_name })}
